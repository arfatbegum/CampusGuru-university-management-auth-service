...
const createDepartmentFromEvent = async (e: AcademicDepartmentCreatedEvent): Promise<void> => {
...
const updateDepartmentFromEvent = async (e: AcademicDepartmentUpdatedEvent): Promise<void> => {
  const academicFaculty = await AcademicFaculty.findOne({ syncId: e.academicFacultyId });
  const payload = {
    title: e.title,
    academicFaculty: academicFaculty?._id
